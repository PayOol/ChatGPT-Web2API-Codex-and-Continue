# ChatGPT Web2API 0.4.11 — Codex integration
...
The [0.4.11 release](https://github.com/PayOol/ChatGPT-Web2API-Continue/releases/tag/v0.4.11) offers **Codex** or **Continue**. Codex defaults to `%LOCALAPPDATA%/Programs/Web2API-Codex`; Continue defaults to `%LOCALAPPDATA%/Programs/Web2API-Continue`. The target is recorded in `installation.json`, and repair keeps that target. Do not reuse one target's root for the other.
...
Before each operation, native `config.toml` bytes are captured **in memory** for the selected and default Codex homes and compared on exit. Existing unrelated catalog slugs are also checked for loss. A change raises `NativeSettingsChanged` and retains the ownership journal; there is no unsafe restoration of the whole file. `native_config_unchanged: true` is returned only when this guard passes.

The same guard captures every foreign provider's stable identity, plus `defaultProvider`, `combos` and top-level `modelAliases`, in memory before any mutation. It compares them on exit even if a request failed. Only provider `initialModelSelection.status` and `initialModelSelection.modelCount` are excluded as known discovery observations; registration IDs, credentials and all other provider fields remain protected. A provider can therefore no longer disappear silently while its catalog row survives. A difference raises `CoexistenceChanged`, retains the owned-entry journal, and reports only bounded provider IDs and whether protected routing changed, never credentials or alias/configuration values. No whole-file restoration is attempted.

Preserve an external baseline for a real deployment as well: these guards detect changes during the operation, not prior drift, and cannot identify which concurrent writer caused them. A provider already absent before a retry is not recreated or attributed to that retry.
...
An integration-OFF CLI sync records a small `catalog_diagnostic` in the journal/result and `logs/codex-provider-diagnostic.json` for CI upload. It contains only a fixed diagnostic code, numeric exit code, catalog-existence/verification booleans and the detach flag. It never includes command output, paths, configuration or credentials. A successful CLI exit without a verified catalog still returns `pending`. The full managed installer must make its private native Codex binary discoverable to OpenCodex when generating a fresh catalog; an existing development-machine catalog is not proof of that first-run path.
