    ...
    def prompt(self, messages: list[dict], *, prior_messages: list[dict] | None = None) -> str:
        ...
        if freeform:
            footer += (
                ...
                "Correct discovery example: const matches = ALL_TOOLS.filter(t => /node_repl/.test(t.name)); text(matches); "
                "Do not use (tools.ALL_TOOLS || []): that manufactures an empty catalog from the wrong variable. "
                ...
                "Images require the image helper, not text or JSON.stringify: "
                "const result = await tools.view_image({path: ABSOLUTE_IMAGE_PATH}); image(result.image_url); "
                "For a tool returning MCP content blocks, forward each image block with image(block). "
                "A data URL printed inside text is not a visible image. Never infer that vision is unavailable "
                "from a result you serialized as text instead of forwarding as an image. "
